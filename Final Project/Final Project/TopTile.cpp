...
void TopTile::hit(Entity* hitThis) {
	if (hitThis->getVelocity().y > 0 && hitThis->getPosition().y + hitThis->getHitBox().getSize().x > position.y && hitThis->getPosition().y + hitThis->getHitBox().getSize().x < position.y + hitBox.getSize().y / 10) {
